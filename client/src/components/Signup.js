...
const Signup = () => {
  ...
  const [validCredentials, setValidCredentials] = useState({ name: "false", password: "false", conpassword: "false" });
  ...
  const onChange = (event) => {
    ...
    if ((name === "name" && value.length < 3) || (name === "password" && value.length < 5)) {
      event.target.classList.add('is-invalid');
    }
    else if ((name === "name" && value.length >= 3) || (name === "password" && value.length >= 5)) {
      if (hasClass) {
    ...
    <div>
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <input type="password" value={credentials.conpassword} name="conpassword" className="form-control" id="conpassword" onChange={onChange} />
          <small id="namevalid" className="form-text text-muted invalid-feedback" style={{ color: "red" }}>
            Password not matching!
          </small>
        </div>
        <button disabled={validCredentials.name === "false" || validCredentials.password === "false" || validCredentials.conpassword === "false"} type="submit" className="btn btn-primary" >Submit</button>
      </form>
    </div>
  )
}
